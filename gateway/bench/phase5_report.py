...
- Co-tenancy: {receipt["disclosure"]["co_tenancy"]} Maximum sampled load1 **{_fmt(co_tenancy["load1_max"], 2)}** vs contamination threshold **{_fmt(co_tenancy["threshold"], 1)}** ({co_tenancy["logical_cpu_count"]} host logical CPUs); max host CPU utilization **{_pct(co_tenancy["cpu_utilization_max"] * 100)}**. The container's provider allocation was 16 CPU cores; it is disclosed separately and is not mixed with the host-level load/CPU denominator. Contaminated stage attempts retained/rerun: **{co_tenancy["contaminated_stage_attempts"]}**.
